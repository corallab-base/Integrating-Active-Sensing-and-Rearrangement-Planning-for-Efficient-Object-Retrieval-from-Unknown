...
def smart_LMP_test():
    ...
    data = np.load("/home/j0k/Project/Imsa/main_code/test_data/test_real_time/test1/9.18.10.49/MCTS*/test_results/test_result_4.npy")
    scene_info = np.load("/home/j0k/Project/Imsa/main_code/test_data/test_real_time/test1/9.18.10.49/MCTS*/test_results/temp_scene3_failed.npy", allow_pickle=True)[0]['scene_info']
    ...
    source_config = plan[0]
    target_config = plan[-1]
    goal_config = plan[-1]
    ...
    smart_LMP_motion(source_treenode, target_treenode, scene_info)


def find_tunnel_side_swipe(cx, cy, radius, rx, ry, robot_width_left, robot_width_right):
    left_theta, right_theta = None, None

    #from the left side
    tri_a_length = radius + robot_width_left
    ...
    left_theta_deduct = np.arcsin(min(1, (radius + robot_width_left)/tri_b_length))
    if cx >= rx:
    ...
    #from the right side
    tri_a_length = radius + robot_width_right
    tri_c_length = get_euc_distance(cx, cy, rx, ry)
    if tri_c_length < tri_a_length:
        return 0, math.pi, 0
    tri_b_length = math.sqrt(tri_c_length**2 - tri_a_length**2)
    left_theta_whole = np.arcsin((cy - ry)/tri_c_length)
    left_theta_deduct = np.arcsin(min(1, (radius + robot_width_right)/tri_b_length))
    ...
    max_length = math.sqrt(max_length_whole**2 - max(robot_width_left, robot_width_right)**2)
...
def smart_LMP_motion_real(source_config, target_config, scene_info):
    grid = []
    for i in range(scene_info[1]):
        temp_arr = []
        for j in range(scene_info[0]):
            temp_arr.append(0)
        grid.append(temp_arr)

    source_treenode = Tree_Node(source_config, target_config, grid, [])
    target_treenode = Tree_Node(target_config, target_config, grid, [])

    #rx, ry = 20, -12.37
    #robot_width_left = 2.2
    #robot_width_right = 5

    rx, ry = 0, -0.19
    robot_width_left = 0.05
    robot_width_right = 0.05
    ...
    print(grasp_angle_degree, relocate_angle_degree)
    ...
    anchor_length = sweep_depth

    #print(smaller_angle, bigger_angle)
    print(source_config[move_index], target_config[move_index], anchor_length)
    for i in range(len(source_config)):
        if i != move_index:
            cx, cy, radius, _ = source_config[i]
            left_theta, right_theta, max_length = find_tunnel_side_swipe(cx, cy, radius, rx, ry, robot_width_left, robot_width_right)
            ...
            if right_theta_degree <= smaller_angle+3 or left_theta_degree >= bigger_angle-3 or max_length > anchor_length:
                pass
            else:
...
def smart_LMP_motion(source_treenode, target_treenode, scene_info):
    step_size = 1.0
    step_rot = 5
    ...
    for i in range(len(source_config)):
        if i != move_index:
            cx, cy, radius, _ = source_config[i]
            left_theta, right_theta, max_length = find_tunnel_side_swipe(cx, cy, radius, rx, ry, robot_width, robot_width)
            left_theta_degree = left_theta / math.pi * 180
            right_theta_degree = right_theta / math.pi * 180
    ...
    #need some good guidence here
        ...
        for distance, offset_list in self.distance_lookup_:
            for ox, oy in offset_list:
                ...
                #may delete
                #if 0 <= gx + ox <= 39 and 0 <= gy + oy <= 39:
                #    while (temp_x < 0 or temp_x > 39 or temp_y < 0 or temp_y > 39):
                #        temp_x = gx + ox + round(random.gauss(0, 1),2)
                #        temp_y = gy + oy + round(random.gauss(0, 1),2)
