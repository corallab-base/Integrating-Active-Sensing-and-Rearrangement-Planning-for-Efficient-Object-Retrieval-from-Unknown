...
if __name__ == '__main__':
    ...
    os.makedirs(new_folder + 'test_results/complete_sensing/')


    # -----------------------------------------------------------------------------------------------------------------------------------------------------

    print("--------------------------- init W feed back ---------------------------")
    seed_state = [0.0]*ik_solver2.number_of_joints
    dof_result = None
    trial = 0
    potential_result = None
    ...
    while not gym.query_viewer_has_closed(viewer):
        if run_mcts and is_target_detected and not need_acquire:
            ...
            if is_plan_success:
                ...
                write_result(new_folder, 'test_results/init_w_feed_back/', scene.num_observation, len(curr_config), ML_MCTS_ins.time_consumption_, ML_MCTS_ins.total_steps_, ML_MCTS_ins.calculate_total_length_travelled(), ML_MCTS_ins.calculate_total_length_displacement(), num_collision_obj_, res_plan)
                ...
                ML_MCTS_ins.MCTS_ins.MCTS_tree_.scene_saver(new_folder + 'test_results/init_sensing/failed_case' + str(scene.num_observation))
        ...
        if need_acquire:
                    ...
                    if (q == 0):
                        ...
                        if (dist1 < 1e-2 or dist3 < 1e-2) and dist2 < 1e-2:
                            ...
                            if is_target_detected:
                                ...
                                # scene save
                                ML_MCTS_ins.MCTS_ins.MCTS_tree_.scene_saver(new_folder + 'test_results/init_w_feed_back/scene_capture' + str(sequence_count) + '.png')
            ...
            while not end_state_collision_free:
                ...
                if is_target_detected: # swept volume observed
                    run_mcts = True
                    if not is_cluster_covered and mcts_out_angle is not None:
    ...
    while not gym.query_viewer_has_closed(viewer):
        if run_mcts and is_target_detected and is_tunnel_covered and not need_acquire:
            ...
            if is_plan_success:
                ...
                res_plan = ML_MCTS_ins.save_planning_results()
                num_collision_obj_ = len(ML_MCTS_ins.track_level_steps_[0][0].check_collision_w_swept())
                write_result(new_folder, 'test_results/init_w_swept/', scene.num_observation, len(curr_config), ML_MCTS_ins.time_consumption_, ML_MCTS_ins.total_steps_, ML_MCTS_ins.calculate_total_length_travelled(), ML_MCTS_ins.calculate_total_length_displacement(), num_collision_obj_, res_plan)
                    ...
                    if (q == 0):
                        ...
                        if (dist1 < 1e-2 or dist3 < 1e-2) and dist2 < 1e-2:
                            ...
                            if is_target_detected:
                                ...
                                # scene save
                                ML_MCTS_ins.MCTS_ins.MCTS_tree_.scene_saver(new_folder + 'test_results/init_w_swept/scene_capture' + str(sequence_count) + '.png')
    ...
    while not gym.query_viewer_has_closed(viewer):
        if is_target_detected:
            ...
            if is_plan_success:
                ...
                res_plan = ML_MCTS_ins.save_planning_results()
                num_collision_obj_ = len(ML_MCTS_ins.track_level_steps_[0][0].check_collision_w_swept())
                write_result(new_folder, 'test_results/init_sensing/', scene.num_observation, len(curr_config), ML_MCTS_ins.time_consumption_, ML_MCTS_ins.total_steps_, ML_MCTS_ins.calculate_total_length_travelled(), ML_MCTS_ins.calculate_total_length_displacement(), num_collision_obj_, res_plan)
                    ...
                    if (q == 0):
                        ...
                        if (dist1 < 1e-2 or dist3 < 1e-2) and dist2 < 1e-2:
                            ...
                            if is_target_detected:
                                ...
                                # scene save
                                ML_MCTS_ins.MCTS_ins.MCTS_tree_.scene_saver(new_folder + 'test_results/init_sensing/scene_capture' + str(sequence_count) + '.png')
...
'''
    ...
    while not gym.query_viewer_has_closed(viewer):
        if run_mcts and is_target_detected and is_tunnel_covered and not need_acquire:
            ...
            save_scene(init2grasp_path, grasp2init_path, rac.obj_pos_list, GT_OBJ_POS_LIST, NUM_OF_OBJECTS, scene_info,
                           rac.target_mesh, rac.obj_mesh, target_obj_pos, GT_TARGET_POS, rac.obstacles_num, W_TARGET,
                           ML_MCTS_ins.unknown_area, ML_MCTS_ins.valid_area, ML_MCTS_ins.potential_centers, "test_results/complete_sensing/", is_plan_success)
            ...
            if is_plan_success:
                ...
                res_plan = ML_MCTS_ins.save_planning_results()
                num_collision_obj_ = len(ML_MCTS_ins.track_level_steps_[0][0].check_collision_w_swept())
                write_result(new_folder, 'test_results/complete_sensing/', scene.num_observation, len(curr_config), ML_MCTS_ins.time_consumption_, ML_MCTS_ins.total_steps_, ML_MCTS_ins.calculate_total_length_travelled(), ML_MCTS_ins.calculate_total_length_displacement(), num_collision_obj_, res_plan)
                ML_MCTS_ins.track_level_steps_[0][0].scene_saver(new_folder + 'test_results/complete_sensing/start_layout.png')
                ML_MCTS_ins.track_level_steps_[-1][-1].scene_saver(new_folder + 'test_results/complete_sensing/final_layout.png')
                break
            else:
                if len(potential_center_cluster) == 0:
                    print("!!!!!!!!!!!!!!Planning Faild without unobserved area!!!!!!!!!!!!!!!!!!!!!")
                    ...
                    if (q == 0):
                        ...
                        if (dist1 < 1e-2 or dist3 < 1e-2) and dist2 < 1e-2:
                            ...
                            if is_target_detected:
                                ...
                                # scene save
                                ML_MCTS_ins.MCTS_ins.MCTS_tree_.scene_saver(new_folder + 'test_results/complete_sensing/scene_capture' + str(sequence_count) + '.png')
