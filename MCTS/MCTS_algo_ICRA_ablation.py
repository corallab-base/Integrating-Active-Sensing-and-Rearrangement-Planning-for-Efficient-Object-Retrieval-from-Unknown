...
class multi_level_MCTS_algo():
	def __init__(self, curr_config, goal_config):
		...
		#AB_NONE*****************************************************************
		#num_objects = len(self.curr_config_)
		#sample_order = random.sample([x for x in range(num_objects)], k = num_objects)

		#new_curr_config = []
		#new_goal_config = []

		#for i in range(len(sample_order)):
		#	new_curr_config.append(self.curr_config_[sample_order[i]])
		#	new_goal_config.append(self.goal_config_[sample_order[i]])
	...
	def global_optimization(self):
		...
		self.track_level_steps_ = [[x[1] for x in recover_plan]]
		self.final_optimized_plan_ = recover_plan
		#ene ablation*******************************************************************************
						...
						for new_index in relocate_object:
							...
							if not temp_relocate_object:
								...
								for upper_index in range(new_index, -1, -1):
									new_regions = selected_leaf_node.propose_new_region(new_index, [index] + [x for x in range(1, new_index)])
									if new_regions: break
								if new_regions:
									for gx, gy in new_regions:
										new_curr_config = deepcopy(curr_config)
...
if __name__ == '__main__':
	...
	if mode == 1:
		...
		write_result('MS_MCTS_CONT_AB_NOPT_FINAL', case_index, len(curr_config), ML_MCTS_ins.time_consumption_, ML_MCTS_ins.total_steps_, ML_MCTS_ins.calculate_total_length_travelled_ICRA(), ML_MCTS_ins.calculate_total_length_displacement_ICRA(), res_plan)
	else:
		for t in range(1, 10):
